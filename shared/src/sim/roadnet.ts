...
/**
 * The road network as a graph (WORLDGEN.md §17.13).
 *
 * §17.5 asked for roads to stop being paint that every consumer
 ...
 * drawn on.
 *
 * **Why this one is derived from the tiles, when §17.11 said not to.** That
 * rule was about GEOMETRY: tracing a raster gives back the staircase, so the
 * shore had to come from the field it was thresholded from. Topology is not
 * damaged by rasterisation — a junction is a junction whatever it is drawn
 * on, and which junction connects to which is exactly as true in the bytes as
 * in the drawing. So the graph is built from the finished tiles and is
 * complete by construction, where the courses (§16) cover only four fifths of
 * the carriageway and would leave a fifth of the city unroutable. What the
 * tiles cannot supply is where an edge RUNS — the paths here are chains of
 * tile centres, staircase and all. Marrying them to the courses is the next
 * step and belongs to the wave that gives the bake a network to emit.
 *
...
export interface RoadNet {
   ...
   * `depth` is not kept at all. Both are the same economy the shore index
   * still owes (§17.12): three `Int32` planes over a 768-square city is seven
   * megabytes of session memory to say something a byte and a short can hold.
   */
  owner: Int16Array;
  fromDir: Uint8Array;
}
