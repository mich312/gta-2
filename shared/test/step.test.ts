...
describe('step', () => {
  ...
  it('never mutates its input state', () => {
    ...
    state = step(state, {}, [{ type: 'spawnPlayer', playerId: 1, name: 'x' }]);
    const before = hashState(state);
    step(state, { 1: scriptedIntent(1, 1) }, [{ type: 'despawnPlayer', playerId: 1 }]);
  ...
  it('spawn is idempotent and despawn removes', () => {
    ...
    state = step(state, {}, [{ type: 'despawnPlayer', playerId: 1 }]);
  ...
  it('players move when given input and stop without it', () => {
    ...
    state = step(state, {}, [{ type: 'spawnPlayer', playerId: 1, name: 'x' }]);
    const x0 = state.players.byId[1]!.pos.x;
    for (let i = 0; i < 30; i++) {
      state = step(state, { 1: { ...NULL_INPUT, seq: i + 1, tick: state.tick, right: true } }, []);
    }
    expect(state.players.byId[1]!.pos.x).toBeGreaterThan(x0);
    for (let i = 0; i < 60; i++) {
      state = step(state, {}, []);
    }
