...
export function computeCamera(snapshot: FullSnapshot | null, localPlayerId: number): Vec2 {
...
export function render(
  ...
  for (const p of snapshot.players) {
    const sx = Math.floor(p.pos.x - cam.x);
    const sy = Math.floor(p.pos.y - cam.y);
    const r = PLAYER_RADIUS;
    const isLocal = p.id === localPlayerId;
    ctx.fillStyle = isLocal
      ? LOCAL_COLOR
      : (REMOTE_COLORS[p.id % REMOTE_COLORS.length] as string);
    ...
    line(
      ...
      sx,
      sy,
      sx + Math.round(Math.cos(p.aimAngle) * (r + 5)),
      sy + Math.round(Math.sin(p.aimAngle) * (r + 5)),
    );
