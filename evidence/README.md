...
| `city-shore-vector.png` | The shore as a curve (WORLDGEN.md §17.11): the waterline shipped as closed rings, stroked in pink over the tile plane it was traced from. Read it at the top left, where the tiles descend in visible 20 px steps and the ring runs as one straight diagonal through the middle of them — the curve was always there, in the field the mask was thresholded from, and the raster was what hid it. On the quay at the right the ring hugs the tile edges instead, correctly: that is built ground, the coastline field has no opinion about it, and the crossing falls back to the middle of the gap. Retake: `pnpm mapgen --shore --scale=20 --crop=600,300,48 --out=evidence/city-shore-vector.png`. |
