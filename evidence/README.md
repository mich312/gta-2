...
| `city-shore-collide.png` | What COLLISION thinks, stippled at eight samples a tile, over the most curved stretch of coast in the city (§43). The red is everywhere the movement solver calls solid, and it follows the drawn waterline — because both now come off the same curve. Retake: `node server/dist/tools/mapgen.js --solid --crop=312,528,26 --out=evidence/city-shore-collide.png`. |
| `city-shore-collide-tiles.png` | The same stretch before §43, drawn from the raster alone: the same water, stopped at tile edges. **The difference between these two pictures is a car's worth of sea** — the renderers had been cutting this coast against the curve since §25, so the waterline had looked right for four waves while a car still stopped at the staircase behind it. Retake: add `--tiles`. |
