...
| `city-seam-review.png` | The §14.1 seam review, four crops left to right: Ravenhill's weave pressed against the Spine's columns with sliver blocks on both sides of a street-less line; the Spine meeting the Old Quarter's 20° fabric at a torn band; Beachfront flipping every district channel into New Suburbs on one line; and New Suburbs stopping dead against Marsh End's empty meadow with no fringe between town and country. The crossings table in §14.1 gives the same review as numbers. Retake with `pnpm mapgen --crop` at the §14.1 coordinates after each seam wave. |
