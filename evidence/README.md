...
| file | what it shows |
|---|---|
| `mapdesign-junction.png` | Seventy-two tiles of the diagonal borough after §52, the three treatments in one frame. Top left is the one signalised crossroads: zebra, stop line and turn arrows on each arm, painted where the junction's own tarmac ends rather than a fixed distance from a curve's crossing point. Everywhere else is give way — the short transverse ladders at the mouth of each minor arm — including at junctions the road curves never described, which are read out of the tile plane instead (§52.2). Retake: `node server/dist/tools/mapgen.js --crop=546,196,72,56 --scale=22 --out=evidence/mapdesign-junction.png`. |
| `mapdesign-junction-close.png` | One T-junction at 110 pixels a tile, which is where the "doubled centre line" of §52.5 was supposed to be. It is not: two streets meeting at about fifteen degrees, each with one correct centre line, converging. What the frame does show is the finished kit at full resolution — edge line at each kerb, one dashed centre line per street, the dash masked out of the junction box, and a give-way ladder at each minor mouth. Retake: `node server/dist/tools/mapgen.js --crop=556,206,12,10 --scale=110 --out=evidence/mapdesign-junction-close.png`. |
| `mapdesign-junction-3d.png` | The same paint in the flyover, with the signal heads standing on it. The crossroads at the top carries its zebra, stop line, turn chevrons and four posts; the posts stand on a kerb the search actually found rather than at a fixed offset that left 156 of them in traffic (§51.6). The lamp's height encodes its colour, frozen at tick 40. Retake: `WAIT_GROUND=40 WAIT_MS=300000 SHOT_TIMEOUT=180000 node ci/shot.mjs "http://localhost:5173/city3d.html?seed=1&fly=1&at=580,222&h=520&pitch=38&tick=40" evidence/mapdesign-junction-3d.png`. |

## §54 — the city as a road network

| file | what it shows |
|---|---|
| `mapdesign-fabric-before.png` / `-after.png` | The Old Quarter, ninety tiles at 14 px each, before and after §54.1. Before: streets 11 tiles apart, so a third of the district's road surface is more than seven tiles across and the two lattice bands have merged into one sheet — buildings scatter across it as confetti, most blocks holding one or two. After: pitch 24, blocks holding six to ten, street corridors you can read, and the merged tarmac down from 31.5% of the district's road to 4.4%. Retake: `node server/dist/tools/mapgen.js --crop=548,150,90,70 --scale=14 --out=evidence/mapdesign-fabric-after.png`. |
| `mapdesign-ring-before.png` / `-after.png` | The ring's central reservation at the top of Ravenhill, 120 tiles at 10 px. Before: pink buildings standing between the two carriageways — 125 of them citywide, more than half of each inside the median, with pavements, reachable from neither side. After: 5,407 tiles of open reservation, because `medianMask` is stamped before the districts run and `buildable` refuses it. Retake: `--crop=340,100,120,60 --scale=10`. |
| `mapdesign-markings.png` | The four carriageway markings in one frame at 30 px/tile: the lane dividers of §54.6 (three dashed lines on the four-tile avenue — centre plus one each side), the parking bays of §54.5 (white rectangles at the kerb), the kerbside restrictions of §54.7 (dim yellow, along the kerbs at the junction mouth, and note there are no bays inside them), and the signalised kit of §53 at the crossroads on the left. Retake: `--crop=470,180,44,34 --scale=30`. |
| `mapdesign-city.png` | The whole city after §54, 2 px a tile. The ring reads as a dual carriageway with a green reservation, the southern parkland no longer carries five arterials in 130 m, and every district's blocks are large enough to hold a frontage. Retake: `node server/dist/tools/mapgen.js --out=evidence/mapdesign-city.png`. |
| `mapdesign-square.png` | Exchange Square after §54.9, 40 tiles at 26 px. The civic square is the paved centre and the block has frontage on it. Before, the landmark's fixed four-tile apron round a thirteen-tile square gave a 21 × 21 clearing box inside a 23 × 18 block, so every building in it was demolished and downtown carried a 414-tile plaza with nothing on it. Retake: `node server/dist/tools/mapgen.js --crop=472,180,40,32 --scale=26 --out=evidence/mapdesign-square.png`. |
