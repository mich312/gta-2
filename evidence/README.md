...
| `city-anywhere.png` | **Anywhere City**, the whole of it: three boroughs on an island group, joined by four bridges, drawn by hand in `shared/data/city-plan.json` and baked to `shared/src/world/city.data.ts`. Port Vasco's docks north-west, Ravenhill's downtown grid north-east with the park in the middle of it, Sunridge's waterfront, suburbs, country and beach across the south. Retake with `pnpm mapgen`. |
...
| `city-anywhere-street.png` | Ground level in Ravenhill: a signalled crossroads, towers on all four corners, the minimap showing the grid it belongs to. `node ci/playLocal.mjs`. |
