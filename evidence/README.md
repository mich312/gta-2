...
| `bug-hairpin-cap.png` / `fixed-hairpin-cap.png` | A Terraces street ending at the shore, at 16 px a tile. Before: the street's course was a hairpin — out along one row of the band's centre tiles and back along the next, because a nearest-neighbour chain that reaches the end of a two-tile-wide ribbon turns round and walks the leftovers home — so the painter stroked two centre lines down one street and, at the turn, a round join half a width past the end of the road, over the pavement and into the sea. After: the chainers never step back or sideways onto the ribbon's other side, and the course trimmer cuts at the sample where the tarmac ends, so the short street keeps its ribbon and ends square. Retake: `--crop=366,384,24 --scale=16`. |
| `bug-crossing-ribbon.png` / `fixed-crossing-ribbon.png` | An industrial street at 20 px a tile. Before: three stray stripes across the block either side — an Old Quarter lattice course overhanging into this borough, crossing the street at twenty degrees and staying on its tarmac for ten tiles, which the precise course trimmer kept as a ribbon. After: a run with course beyond both its ends is a crossing, not a street, and needs four widths to count. Retake: `--crop=524,258,20 --scale=20`. |
| `bug-band-overshoot.png` / `fixed-band-overshoot.png` | A Terraces T-junction at 14 px a tile. Before: the contour band coming up from the south overshoots the street it meets by two tiles — a 2×3 knob on the far kerb with the band's ribbon curling into it. The stub walk could not see it because a crossing of two three-wide streets was not a junction to it: the junction test wanted over-wide on both diagonals, which a narrow crossing square is not. After: a junction is over-wide on both axes and not a diagonal road (one diagonal long, the other no wider than the road), and the overshoot is a stub like any other. Retake: `--crop=530,536,28 --scale=14`. |
| `bug-cape-stub.png` / `fixed-cape-stub.png` | The Docks shore at 14 px a tile. Before: a curved street down the cape to the beach, eight tiles of dead end; one of some sixty short dead ends the widened junction test lets the stub walk reach. After: gone, the block built over it. Retake: `--crop=448,354,28 --scale=14`. |
| `bug-unmarked-grid.png` / `fixed-unmarked-grid.png` | The Spine's grid at 8 px a tile, the ring road below it for comparison. Before: every grid street is a bare dark band — no kerb casing, no centre line — because the axis-aligned grid was the one fabric that carved its streets as rectangles and recorded no course, so the ribbon painter never reached them: a fifth of the city's carriageway, all of The Spine's grid and most of Ravenhill's. After: each lattice line is a course like every other fabric's, and the trim pass clips it to what was carried. Uncovered carriageway 19.9% → 9.5%. Retake: `--crop=300,560,64 --scale=8`. |
| `bug-avenue-gap.png` / `fixed-avenue-gap.png` | The Spine's grid at 8 px a tile, where South Sound Bridge lands. Before: two authored avenues that never met — the bridge's road landed at 214,486 and The Esplanade began at 228,470, fourteen tiles away — so each ended inside a block and the grid's own streets carried the traffic between them. After: The Esplanade begins at the bridge landing; one avenue from the water to the seafront. A plan fix, one line. Retake: `--crop=200,440,64 --scale=8`. |
| `bug-ribbonless-track.png` / `fixed-ribbonless-track.png` | The Spine's south-west shore at 12 px a tile. Before: the track the shore pass lays to reconnect a stranded piece of road, drawn per tile — a two-wide staircase down the beach with no casing and no marking, the largest stretch of ribbonless carriageway in the city. After: every carriageway no ribbon reaches gets a course recovered from its own medial tiles (`recoverCourses`), and the painter strokes a lane through the steps. Retake: `--crop=246,566,32 --scale=12`. |
| `bug-ribbonless-quay.png` / `fixed-ribbonless-quay.png` | Vasco Heights' east quay at 14 px a tile. Before: the quay street had no ribbon because the esplanade's course, chained round the corner of the quay, was cut by the trim as a crossing. After: recovered like the track above. Ribbonless carriageway 19.9% → 4.9% over loops 4 and 6. Retake: `--crop=160,236,24 --scale=14`. |
| `bug-walk-ends.png` / `fixed-walk-ends.png` | Ravenhill Park at 8 px a tile: a walk from the coast road down to the ring junction. Before: its ribbon ran only along the middle third and both ends were bare steps, because the course trimmer held a walk to pavement alone and a two-wide meander is a staircase whose corners a straight sample lands on the lawn between. After: a walk's ground is the pavement it carved and the lawn either side, the ground the painter already clips it to. Path courses 20 → 32, 572 → 914 tiles. Retake: `--crop=430,20,64 --scale=8`. |
| `bug-doubled-band.png` / `fixed-doubled-band.png` | The Spine's south shore at 9 px a tile. Before: two ribbons stacked into one six-wide sheet — the contour fabric's innermost shore band carved three rows from a seam street laid one pass earlier, which its parallel-road probe could not see because it read the tiles as they were before the esplanade. After: the probe reads the tiles as the borough finds them, and the band keeps off the seam street. The widest junction patch in the city (277 tiles) gone; patches over 80 tiles 14 → 7. Retake: `--crop=214,500,52 --scale=9`. |
| `bug-doubled-lane.png` / `fixed-doubled-lane.png` | The Beachfront–New Suburbs seam at 9 px a tile. Before: a crescent lane ran a tile from the seam street for a dozen tiles — two ribbons with a sliver of pavement between — because the lane's doubled-up tests are global to the whole lane. After: the wave is sampled against the tiles as they stand, a stretch running beside another road is left uncarved, and the lane is carried on from either end until it meets that road as a junction rather than stopping three tiles short of it. Retake: `--crop=450,450,56 --scale=9`. |

## Map loops — as the game shows it

| file | what it shows |
|---|---|
| `bug-kerb-across-junction.png` / `fixed-kerb-across-junction.png` | The Spine's south shore in the 3D flyover, at tile 242,510. Before: pale kerb lines running across the tarmac at both junctions — every course's edge ring is stroked over everything, and where a course crosses carriageway that no course's interior repaint reaches (a junction sheet, the wedge where a diagonal merges into a street) the ring lay across the road like a kerb painted through the crossing. After: the painter lays bare asphalt over every junction disc and over every carriageway tile no ribbon's interior reaches, before the interiors and the dashes go back on. Retake: `pnpm --filter client dev`, then `node ci/shot.mjs "http://localhost:5173/city3d.html?fly=1&at=242,510&pitch=55&h=170" evidence/<file>`. |
| `bug-stair-kerb.png` / `fixed-stair-kerb.png` | The ring road through the south shore park, straight down from the 3D flyover at tile 340,650. Before: a saw-tooth of dark steps outside every kerb line — each road tile under the curved ribbon was filled asphalt whole, and the corners the curve left outside the kerb stayed dark. After: a road tile the ribbon encloses is painted as the ground beside the road and the kerb casing is drawn over it, so the kerb follows the curve and the corner outside it is lawn. Retake: `pnpm --filter client dev`, then `WAIT_GROUND=24 node ci/shot.mjs "http://localhost:5173/city3d.html?fly=1&at=340,650&pitch=10&h=300" evidence/<file>`. |
| `bug-pocket-sheet.png` / `fixed-pocket-sheet.png` | A Beachfront crossroads by the river, straight down from the 3D flyover at tile 388,403. Before: the block's corner is a sheet of bare tarmac — the excess of the carved junction beyond the two ribbons that cross there, road no course reaches. After: the bake's `trimPockets` gives such a pocket back to the ground beside it, and the corner is a paved quay square with a kerb, the crossing exactly the union of its two roads. Retake: `pnpm --filter client dev`, then `WAIT_GROUND=24 node ci/shot.mjs "http://localhost:5173/city3d.html?fly=1&at=388,403&pitch=10&h=220" evidence/<file>`. |
| `bug-quay-fan.png` / `fixed-quay-fan.png` | The Beachfront at tile 465,410, straight down from the 3D flyover. Before: the street north of the crossroads widens into a fan of bare tarmac running out to the beach — a quay apron no course covers, kept by the waterfront rule because nothing else put a road within five tiles of that shore. After: a two-wide quay lane with its own kerb and dash runs from the street's end along the water, and the apron behind it is pavement and lawn. Retake: `pnpm --filter client dev`, then `WAIT_GROUND=24 node ci/shot.mjs "http://localhost:5173/city3d.html?fly=1&at=465,410&pitch=10&h=300" evidence/<file>`. |
| `bug-flat-roofs.png` / `fixed-flat-roofs.png` | The Terraces at the game's own pitch, from the 3D flyover at tile 560,520. Before: every house the same flat grey lid with a white parapet. After: three houses in four wear a pitched roof — terracotta or slate by hash, ridge along the long side, eaves past the walls, a chimney on every other ridge — and the flat-roofed rest keep their parapets, so the street is a street rather than a rank of cottages. Retake: `pnpm --filter client dev`, then `WAIT_GROUND=24 node ci/shot.mjs "http://localhost:5173/city3d.html?fly=1&at=560,520&pitch=42&h=260" evidence/<file>`. |
| `bug-box-skyline.png` / `fixed-box-skyline.png` | Vantage Row's blocks at the game's own pitch, from the 3D flyover at tile 470,200. Before: every tall block one box with one flat lid. After: the tall blocks step — their ends stand at three fifths of the core, the core rimmed where it rises — and the tallest carry a mast; the volume grid steps with them, so the roof you land on is the roof you see. Retake: `pnpm --filter client dev`, then `WAIT_GROUND=24 node ci/shot.mjs "http://localhost:5173/city3d.html?fly=1&at=470,200&pitch=42&h=300" evidence/<file>`. |
| `bug-one-facade.png` / `fixed-one-facade.png` | The Docks' sheds close up, from the 3D flyover at tile 120,266. Before: the same window grid and the same shopfront as every tower downtown. After: the industrial facade — a high strip of glass over blank wall, roller doors at the yard — while downtown keeps its curtain wall, the houses get front doors and sills, and the shops a fascia. Retake: `pnpm --filter client dev`, then `WAIT_GROUND=24 node ci/shot.mjs "http://localhost:5173/city3d.html?fly=1&at=120,266&pitch=60&h=160" evidence/<file>`. |
| `bug-bare-shopfront.png` / `fixed-bare-shopfront.png` | The Docks gun shop from the 3D flyover at tile 80,262. Before: the shop is a courtyard room with a gap in its street wall and nothing on the outside to say so. After: a sign across the doorway's lintel and an awning out over the pavement, both in the shop's red. Retake: `pnpm --filter client dev`, then `WAIT_GROUND=24 node ci/shot.mjs "http://localhost:5173/city3d.html?fly=1&at=80,262&pitch=50&h=150" evidence/<file>`. |
| `bug-walk-blob.png` / `fixed-walk-blob.png` | The walks' hub beside the ring road in the south shore park, from the 3D flyover at tile 326,630. Before: four two-tile stone strokes, two of them the same walk twice, pooled into a slab six tiles across with a straight clipped edge along the wood. After: the duplicate walks dropped in the bake and the stroke narrowed to a tile and a quarter — paths meeting on a lawn. Retake: `pnpm --filter client dev`, then `WAIT_GROUND=24 node ci/shot.mjs "http://localhost:5173/city3d.html?fly=1&at=326,630&pitch=45&h=220" evidence/<file>`. |
| `bug-bare-roofs.png` / `fixed-bare-roofs.png` | The Docks' sheds from the 3D flyover at tile 124,262. Before: the same slate and the same grey boxes as every roof in the city. After: gravel roofs with skylights on the sheds; elsewhere tar on the shops with a water tank among the plant, felt on the houses. Retake: `pnpm --filter client dev`, then `WAIT_GROUND=24 node ci/shot.mjs "http://localhost:5173/city3d.html?fly=1&at=124,262&pitch=42&h=220" evidence/<file>`. |
