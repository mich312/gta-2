...
| `city-roadnet.png` | The road network as a graph (WORLDGEN.md §40): every street between two junctions stroked in cyan along the tiles the flood ran through, every junction a yellow dot. 940 nodes and 1,764 streets stand in for the 102,987 drivable tiles routing used to search, and the picture is the coverage claim — there is no carriageway in the crop the graph does not run down. Retake: `pnpm mapgen --net --crop=300,180,120 --out=evidence/city-roadnet.png`. |
